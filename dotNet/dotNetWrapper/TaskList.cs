...
namespace Abbyy.CloudOcrSdk
{
    ...
    class TaskList
    {
        private Dictionary<TaskId, OcrTask> allTasks = new Dictionary<TaskId, OcrTask>();
        ...
        public void Start( object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                while (!shouldStop)
                {
                    ...
                    if (hasJobs)
                    {
                        OcrTask[] serverTasks = _restClient.ListTasks(lastCheckTime);

                        lock (allTasks)
                        {
                            foreach (OcrTask task in serverTasks)
                            {
                            ...
                            foreach (OcrTask task in allTasks.Values)
                            {
        ...
        public void AddTask(OcrTask task)
        {
            lock (allTasks)
            {
        ...
        public bool IsTaskFinished(TaskId taskId)
        {
            ...
            if (status == TaskStatus.Unknown || OcrTask.IsTaskActive(status))
                return false;
            else
                return true;
        }
        ...
        public OcrTask GetTask(TaskId taskId)
        {
            lock (allTasks)
            {
